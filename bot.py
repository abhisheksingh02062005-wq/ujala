...
from config import (
    ...
    POINTS_PER_REFERRAL, POINTS_PER_NUMBER
)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        ...
        if referred_by:
            ...
            if referrer and not referrer.get("is_banned"):
                await db.update_points(referred_by, POINTS_PER_REFERRAL)
                new_ref_pts = await db.update_referral_points(referred_by, POINTS_PER_REFERRAL)
                try:
                    await context.bot.send_message(
                        referred_by,
                        f"🎉 <b>Referral Bonus!</b>\n\n"
                        f"🙋 <b>{first_name}</b> just joined using your link!\n"
                        f"💰 You earned <b>+{POINTS_PER_REFERRAL} referral points!</b>\n"
                        f"🎯 Referral Points: <b>{new_ref_pts}</b> (use to scan numbers)",
    ...
    if is_new:
        greet = (
            ...
            f"🎯 <b>Submit numbers</b> → Earn +{POINTS_PER_NUMBER} point each\n"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Submit number ──────────────────────────────────────────────────────
        ...
        if not os.path.isfile(PACK_IMAGE_PATH):
            ...
            return
        # Gate: user must have at least 1 referral point to scan
        ref_pts = await db.get_referral_points(user_id)
        if ref_pts < 1:
            bot_username = (await context.bot.get_me()).username
            ref_link = f"https://t.me/{bot_username}?start=ref_{user_id}"
            await query.edit_message_text(
                f"🚫 <b>Not Enough Referral Points!</b>\n\n"
                f"━━━━━━━━━━━━━━━━\n"
                f"🎯 Scanning a number costs <b>1 Referral Point</b>\n"
                f"💎 Your Referral Points: <b>{ref_pts}</b>\n"
                f"━━━━━━━━━━━━━━━━\n\n"
                f"📢 <b>How to get Referral Points?</b>\n"
        ...
        await query.edit_message_text(
            ...
            f"🎯 Cost: <b>1 Referral Point</b> | Your balance: <b>{ref_pts}</b>",
            parse_mode=ParseMode.HTML,
    ...
    # ── My points ──────────────────────────────────────────────────────────
        ...
        await query.edit_message_text(
            ...
            f"🎯 Submit a number → +{POINTS_PER_NUMBER} point\n"
    ...
    # ── How it works ───────────────────────────────────────────────────────
        ...
        await query.edit_message_text(
            f"📖 <b>How It Works</b>\n\n"
            f"It's simple! Follow these steps:\n\n"
            f"1️⃣ Tap <b>Submit Number & Earn</b>\n"
            f"2️⃣ Enter a <b>10-digit mobile number</b>\n"
            f"3️⃣ The bot registers on the Ujala website\n"
            f"4️⃣ An <b>OTP will be sent</b> to that number via SMS\n"
            f"5️⃣ Enter the OTP in the bot\n"
            f"6️⃣ Bot does the rest automatically! 🤖\n"
            f"7️⃣ <b>You earn +{POINTS_PER_NUMBER} point!</b> 🎉\n\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"⚠️ <b>Important Notes:</b>\n"
            f"• If the number is <b>already registered</b> on Ujala → No points lost, just try another number\n"
            f"• Each number can only be used <b>once</b>\n"
            f"• OTP expires quickly — enter it fast!\n"
            f"━━━━━━━━━━━━━━━━\n\n"
            f"💡 <b>Earn more with referrals!</b>\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── User: waiting for phone ────────────────────────────────────────────
        ...
        # Deduct 1 referral point before starting
        ref_pts_now = await db.get_referral_points(user_id)
        if ref_pts_now < 1:
            await db.clear_session(user_id)
            await update.message.reply_text(
                f"🚫 <b>No Referral Points!</b>\n\nYou need referral points to scan numbers.\n"
        ...
        # Step 1: Register
        reg_result, cookies = await loop.run_in_executor(None, api.api_register)

        if reg_result.get("statusCode") != 200:
            msg = reg_result.get("message", "Unknown error")
            await db.clear_session(user_id)
            # Refund referral point — nothing actually happened
            await db.update_referral_points(user_id, 1)
            if api.is_already_used_error(reg_result):
                ...
                await processing_msg.edit_text(
                    ...
                    f"The Ujala website didn't respond. Your referral point was refunded.\n\n"
        ...
        if otp_result.get("statusCode") not in (200, 201):
            await db.clear_session(user_id)
            # Refund referral point — OTP never arrived
            await db.update_referral_points(user_id, 1)
            if api.is_already_used_error(otp_result):
                await db.log_usage(user_id, phone, False, 0)
                await processing_msg.edit_text(
                ...
                await processing_msg.edit_text(
                    ...
                    f"Ujala couldn't send the OTP. Your referral point was refunded.\n\n"
                    f"Please try again.",
                    parse_mode=ParseMode.HTML,
                    reply_markup=main_menu_keyboard(user_id),
                )
        ...
        # Save session — store both reg cookies (for resend) and post-OTP cookies
    ...
    # ── User: waiting for OTP ─────────────────────────────────────────────
        ...
        # Restore original integer type — HMAC signing is type-sensitive
        try:
        ...
        verify_result, cookies3 = await loop.run_in_executor(None, verify_fn)

        if verify_result.get("statusCode") not in (200, 201):
            msg = verify_result.get("message", "Unknown error")
            await processing_msg.edit_text(
                f"❌ <b>Wrong OTP!</b>\n\n"
        ...
        spin_result, cookies4 = await loop.run_in_executor(None, spin_fn)

        if spin_result.get("statusCode") not in (200, 201):
            await db.log_usage(user_id, phone, False, 0)
            await db.clear_session(user_id)
            # Refund referral point — Ujala's fault, not user's
            await db.update_referral_points(user_id, 1)
            await processing_msg.edit_text(
                f"⚠️ <b>Spin Failed</b>\n\n"
                f"The Ujala website didn't respond at the spin step.\n\n"
                f"✅ <b>Referral point refunded!</b>\n"
        ...
        claim_result = await loop.run_in_executor(None, claim_fn)

        # Award point
        new_pts = await db.update_points(user_id, POINTS_PER_NUMBER)
        await db.log_usage(user_id, phone, True, POINTS_PER_NUMBER)
        await db.clear_session(user_id)
        ...
        await processing_msg.edit_text(
            ...
            f"━━━━━━━━━━━━━━━━\n\n"
            f"💰 <b>+{POINTS_PER_NUMBER} point earned!</b>\n"
            f"🏆 Your total: <b>{new_pts} points</b>\n\n"
            f"🔄 <i>Submit another number to earn more!</i>",
            parse_mode=ParseMode.HTML,
            reply_markup=main_menu_keyboard(user_id),
        )
